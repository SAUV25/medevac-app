...
import { Check, AlertCircle, Send, Calendar, Clock, User, History, Eye, Printer, X, Edit, Trash2, Plus, Stethoscope } from 'lucide-react';
// FIX: Corrected import path for types to be relative.
import { ChecklistItems, VerificationRecord } from '../types';
...
type Status = 'OK' | 'Problème' | null;
type ChecklistState = Record<string, Status>;
...
const NurseVerification: React.FC = () => {
    ...
    const history = useMemo(() => verifications.filter(v => v.type === 'Infirmier'), [verifications]);
    ...
    const handleEdit = (record: VerificationRecord) => {
        ...
        setChecklist(record.data as ChecklistState);
    ...
    const handleSubmit = () => {
        if (editingRecord) {
            ...
            const totalItems = Object.values(nurseChecklistItems).flat().length;
    ...
    const handleStatusChange = (item: string, status: 'OK' | 'Problème') => {
        setChecklist(prev => ({
            ...prev,
    ...
    const handlePrint = (record: VerificationRecord) => {
        ...
        let printContent = `
            <html>
            <head>
                <title>Fiche Contrôle Infirmier - ${record.vehicleId}</title>
                <style>
                ...
                <div class="container">
                    <div class="header">
                        ...
                        <div class="header-right">
                            <h1 class="doc-title">Contrôle Infirmier</h1>
                    ...
                    <div class="info-grid">
                        ...
                        <div class="info-item">
                            ...
                            <span class="info-value">Cadre Santé</span>
                        </div>
                    </div>
        ...
        Object.entries(nurseChecklistItems).forEach(([category, items]) => {
            printContent += `
                <div class="category">
        ...
        printContent += `
                    ...
                    <div class="signatures">
                        <div class="sig-box">
                            <span class="sig-title">Signature Infirmier</span>
                            <div style="font-family: 'Brush Script MT', cursive; font-size: 18px; color: #0056b3;">${record.user}</div>
                        </div>
                        <div class="sig-box">
                            <span class="sig-title">Visa Cadre de Santé / Pharmacie</span>
                        </div>
                    </div>
                </div>
            </body>
            ...
            return (
                <div className="flex justify-center items-start pt-10 md:pt-16 h-full">
                    <Card className="max-w-md w-full shadow-lg border-t-4 border-primary">
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-6 flex items-center"><Stethoscope size={28} className="mr-3 text-primary"/>Checklist Infirmier</h2>
        ...
        return (
            <Card className="shadow-lg">
                <div className="flex justify-between items-center border-b pb-4 mb-6 dark:border-gray-700">
                    <div>
                        <h2 className="text-2xl font-bold text-primary flex items-center"><Stethoscope size={24} className="mr-2"/> {verificationInfo.vehicleId}</h2>
                        <p className="text-sm text-gray-500">Contrôle Matériel Médical par {verificationInfo.user}</p>
                    </div>
                    <button onClick={handleCancel} className="btn-secondary text-sm">Annuler</button>
                </div>

                <div className="space-y-8">
                    {Object.entries(nurseChecklistItems).map(([category, items]) => (
                ...
                <div className="mt-8 bg-yellow-50 dark:bg-yellow-900/10 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800">
                     <label className="block text-sm font-bold text-yellow-800 dark:text-yellow-500 mb-2 flex items-center"><AlertCircle size={16} className="mr-2"/> Observations / Matériel Manquant</label>
                     <textarea value={comments} onChange={(e) => setComments(e.target.value)} rows={3} className="input w-full border-yellow-300 focus:border-yellow-500 focus:ring-yellow-500" placeholder="Décrivez les problèmes éventuels ou le matériel à réapprovisionner..."/>
                </div>
                ...
                <Card className="w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in fade-in zoom-in duration-200">
                    <div className="flex justify-between items-start border-b pb-4 mb-6 dark:border-gray-700">
                        <div>
                            <h2 className="text-xl font-bold text-gray-800 dark:text-white flex items-center">
                                <Stethoscope className="mr-2 text-primary" size={24}/> Détails Contrôle {selectedRecord.vehicleId}
                            </h2>
                    ...
                    <div className="mb-6 flex items-center justify-between bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border dark:border-gray-700">
                        ...
                        <span className={`px-4 py-1.5 rounded-full text-sm font-bold uppercase tracking-wide flex items-center ${hasProblems ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                            {hasProblems ? <><AlertCircle size={16} className="mr-2"/> Problèmes Détectés</> : <><Check size={16} className="mr-2"/> Matériel Conforme</>}
                        </span>
                    </div>
    ...
    return (
        <div className="space-y-6">
            <h1 className="text-3xl font-bold dark:text-gray-100 flex items-center"><Check size={32} className="mr-3 text-primary"/>Vérification Infirmier</h1>
